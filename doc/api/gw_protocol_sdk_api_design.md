...
| 版本号 | 日期 | 修改人 | 变更说明 |
| --- | --- | --- | --- |
| V1.2 | 2026-05-20 | 姜俊丞 | 收敛参数下发、自描述文件、文件传输、软件升级、恢复出厂设置和设备重启的接口口径：参数下发改为预置/执行/取消；参数校验迁移到上位机；自描述复用通用文件接口；文件传输去除取消抽象；升级改为控制接口配合写文件；恢复出厂设置和设备重启按遥控扩展点两阶段处理。完整差异见 `gw_protocol_sdk_api_design_v1.1_to_v1.2_diff.md`。 |
