...
当前代码公开并实现接口设计文档中的核心函数族，以及三套协议库的配置校验函数。已覆盖会话生命周期、配置校验、运行状态查询、运行选项设置、总召、电度量召唤、命令控制、单点读取、时钟同步、读时钟、参数读取、参数写入、定值区切换、终端自描述、文件目录、文件读写、文件传输状态查询、文件传输取消、程序升级、升级取消和原始 ASDU 发送接口。参数一致性比对、模板校验和差异定位由上位机基于参数读取结果实现。
